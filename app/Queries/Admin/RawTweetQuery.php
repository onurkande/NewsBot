...
class RawTweetQuery
{
    private const DEFAULT_SORT = 'tweeted_at';
    ...
    public function forIndex(array $filters = []): array
    {
        ...
        return [
            ...
            'selectionKey' => 'raw-tweets',
        ];
    }

    public function paginate(array $filters): LengthAwarePaginator
    {
        $query = RawTweet::query()
        ...
        match ($filters['filter']) {
            ...
            'pending' => $query->where('is_processed', false),
            default => null,
        };

        if ($filters['sort'] === 'interactions') {
            $query->orderByRaw('(like_count + retweet_count + view_count) ' . $filters['dir']);
        } else {
            $query->orderBy('tweeted_at', $filters['dir']);
        }
    ...
    public function filterOptions(): array
    {
        return [
            ...
            ['value' => 'processed', 'label' => 'İşlenmiş'],
    ...
    {
        $columns = [
            ['label' => 'Tarih', 'key' => 'tweeted_at'],
            ['label' => 'Etkileşim', 'key' => 'interactions'],
        ];
        ...
        return collect($columns)->map(function (array $column) use ($filters) {
            ...
            return [
                ...
                'url' => route('admin.raw-tweets.index', array_filter([
    ...
    private function normalizeFilters(array $filters): array
    {
        ...
        if (! in_array($filter, ['all', 'processed', 'pending'], true)) {
            $filter = 'all';
        }

        if (! in_array($sort, ['tweeted_at', 'interactions'], true)) {
