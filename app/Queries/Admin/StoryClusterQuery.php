...
class StoryClusterQuery
{
    private const DEFAULT_SORT = 'last_updated_at';
        ...
        if ($filters['q'] !== '') {
            ...
            $query->where(function ($builder) use ($search) {
                $builder->where('title', 'like', "%{$search}%")
                    ->orWhere('cluster_hash', 'like', "%{$search}%");
            });
        }
    ...
    {
        $columns = [
            ['label' => 'Son Güncelleme', 'key' => 'last_updated_at'],
            ['label' => 'Skor', 'key' => 'story_score'],
            ['label' => 'Tweet Sayısı', 'key' => 'items_count'],
        ];
        ...
        return collect($columns)->map(function (array $column) use ($filters) {
            ...
            return [
                ...
                'url' => route('admin.story-clusters.index', array_filter([
    ...
    private function normalizeFilters(array $filters): array
    {
        ...
        if (! in_array($filter, ['all', 'open', 'selected', 'published', 'ignored'], true)) {
            $filter = 'all';
        }

        if (! in_array($sort, ['last_updated_at', 'story_score', 'items_count'], true)) {
